...
IplImage* cvRotation(IplImage* Object_Image,double Angle_val)
{
	...
	CvPoint2D32f Center = cvPoint2D32f(w_src / 2, h_src / 2);  

	cv2DRotationMatrix(Center,(-1)* Angle_val, 1.0, &map_matrix);
...
IplImage* cvMoveImage(IplImage* Object_Image,int width,int height)
{
	...
	CvScalar Temp;
	IplImage* Dist_Image=cvCreateImage(cvSize(Object_Image->width,Object_Image->height),8,3);

	if(width>=0&&height>=0)
	{
		for(i=0;i<Object_Image->height-height;i++)
		{
			for(j=0;j<Object_Image->width-width;j++)
			{
				Temp=cvGet2D(Object_Image,i,j);
				cvSet2D(Dist_Image,i+height,j+width,Temp);
			}
		}
	}
	else if(width>=0&&height<0)
	{
		for(i=-height;i<Object_Image->height;i++)
		{
			for(j=0;j<Object_Image->width-width;j++)
			{
				Temp=cvGet2D(Object_Image,i,j);
	...
	else if(width<0&&height>=0)
	{
		for(i=0;i<Object_Image->height-height;i++)
		{
			for(j=-width;j<Object_Image->width;j++)
			{
				Temp=cvGet2D(Object_Image,i,j);
				cvSet2D(Dist_Image,i+height,j+width,Temp);
			}
...
IplImage* cvRotationMoveImage(IplImage* Object_Image,CvRatation_Parameter RatationMove_val )
{
	...
	CvScalar Temp;
	IplImage* Source_Image=cvCreateImage(cvSize(Object_Image->width,Object_Image->height),8,3);

	if(RatationMove_val.Center_X>=0&&RatationMove_val.Center_Y>=0)
	{
		for(i=0;i<Object_Image->height-RatationMove_val.Center_Y;i++)
		{
			for(j=RatationMove_val.Center_X;j<Object_Image->width;j++)
			{
				x=(i+RatationMove_val.Center_Y)*sin(RatationMove_val.Thata)+(j+RatationMove_val.Center_X)*cos(RatationMove_val.Thata);
				y=(i+RatationMove_val.Center_Y)*cos(RatationMove_val.Thata)-(j+RatationMove_val.Center_X)*sin(RatationMove_val.Thata);
				a=(int)x;
				b=(int)y;
				/*if(b<0||a<0)
				{
				...
				//Temp=cvGet2D(Object_Image,i,-j);
				//cvSet2D(Source_Image,b,a,Temp);
			}
			//printf("%d,%d,",a,b);
		}
	}
	/*else if(RatationMove_val.Center_X>=0&&RatationMove_val.Center_Y<0)
	{
		for(i=-RatationMove_val.Center_Y;i<Object_Image->height;i++)
		{
			for(j=0;j<Object_Image->width-RatationMove_val.Center_X;j++)
			{
				x=(i+RatationMove_val.Center_Y)*sin((-1.0)*RatationMove_val.Thata)+(j+RatationMove_val.Center_X)*cos((-1.0)*RatationMove_val.Thata);
				y=(i+RatationMove_val.Center_Y)*cos((-1.0)*RatationMove_val.Thata)-(j+RatationMove_val.Center_X)*sin((-1.0)*RatationMove_val.Thata);
				Temp=cvGet2D(Object_Image,i,j);
				cvSet2D(Source_Image,y,x,Temp);
			}
		}
	}
	else if(RatationMove_val.Center_X<0&&RatationMove_val.Center_Y>=0)
